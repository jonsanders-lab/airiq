...
- Blitz Mode enhancements (session 11): (1) UNDO logged stop — DELETE /api/blitz/log/:stopId clears logged/outcome/notes; "↺ Undo" button on logged cards. (2) Territory now a readable chip (surface2 bg + border + C.text) instead of near-invisible dim text. (3) MANUAL stops — floating "＋ ADD UNPLANNED STOP" button + form; POST /api/blitz/stops/manual creates role='MANUAL' stop with auto stop_number M1/M2…; renders at bottom of primary list with a purple "M" badge. (4) LOAD-NEW / ARCHIVE — uploading a new file while active archives the old session (active=FALSE, data preserved) after an "Archive & Start New" warning modal; GET /api/blitz/sessions lists all sessions (id,name,created_at,active,totalStops,loggedStops); "Archived Blitzes" list in the leader upload panel; "Load New" button in the active blitz header. (5) EXPORT — GET /api/blitz/export/:sessionId streams an XLSX (19 cols incl Assigned Rep); "📥 Export" in the active header + per-session in the archived list; triggered by direct URL navigation to bypass the service worker. (6) REP ASSIGNMENT + MONDAY PUSH — an "Assign Reps to Groups" modal after xlsx parse maps each group→rep (from REPS); stored in blitz_sessions.group_assignments JSONB; on log, the server looks up the group's assigned rep and calls mondayUpsertProspect (branch derived from stop territory via territoryToBranch city includes() → "{City} Air Compressor"). ST Inbound Lead Form checkbox gates LOG STOP for opp outcomes (Membership/Equipment/Service Lead/Piping/VR/Appointment) — same form link as regular Field Log.
- Blitz Mode bug fixes (session 11): group selector no longer prepends "Group " to names that already contain it (fixed in selector, assign modal, manual modal, empty state); "Load New" moved to the persistent sub-tab row so it always shows to leaders; rep assignment now REQUIRED — START BLITZ disabled until every detected group has a rep (guard in submitUpload + inline hint) so group_assignments can't save empty. territoryToBranch corrected to "{City} Compressor" (matches the branch labels regular Field Log sends to monday.com); blitz monday push skipped entirely when the group's assigned rep is Morty or Hudson.
- Blitz Mode contact enhancements (session 11): (1) Unplanned-stop form now has full contact fields (company req + contact/title/phone/mobile/email/website/address/notes). (2) "📷 SCAN BUSINESS CARD" on the unplanned-stop form (id=blitzCardInput, scanManualCard) reuses the MY LOG scanner — posts image to /api/claude and fills the form. (3) "✏️ Edit Contact" inline editor on every stop card → PATCH /api/blitz/stops/:stopId/contact (contact_name,title,phone,email,mobile,website,address); added blitz_stops.email + blitz_stops.mobile columns. (4) Monday push now falls back to the logging rep — client sends repName in POST /api/blitz/log body; server uses group_assignments[group] || repName so pre-fix blitzes (empty group_assignments, e.g. Ohio Magnetics) still tag a rep.
